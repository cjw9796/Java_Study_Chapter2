<%@page import="com.Voting.PollWrite"%>
...
<body>
	<!-- pollRead.jsp 에서 넘어온 투표한 항목을 받아서 득표수를 증가시켜 텍스트 파일에 저장한다. -->
	...
	<%
	  ...
	  if(temp != null && temp.trim().length() != 0){
		  ...
		  String filepath = application.getRealPath("/") + "poll.txt";
			...
			//넘어온 투표 데이터가 숫자인 경우 정상적인 투표범위의 데이터인가 검사한다. 
			if(result >= 1 && result <= itemCount){
				// 여기까지 왔다면 정상적인 투표 데이터가 넘어왔다. 투표한 항목의 득표수를 증가시켜 텍스트 파일에 저장 
				// 득표수를 1씩 증가. 어디 위치에 증가시킬지 확인한다. 
				
				int index = result + itemCount;
				// 1 증가된 득표수를 다시 Arraylist에 index 번째 위치에 넣어준다. 
				// 득표수를 1증가시킨 다음에 -> poll Arraylist index번째 데이터를 1 증가시킨다.
				
				result = Integer.parseInt(poll.get(index))+1;
				
				
				// 숫자로 변환해서 계산을 했기 때문에 파일에 다시 저장될 때는 문자열
				// poll.set(index, String.valueOf(result));
				// poll.set(index, String.format("%s",result));
				
				poll.set(index,result + "");
				
				//poll => 텍스트 파일에 저장할 수 있도록 메서드를 실행 
				...
				//넘어온 투표 데이터가 정상적인 투표범위가 아니거나 숫자가 아니거나 오류메시지를 출력하고 pollRead.jsp로 돌려보낸다.
