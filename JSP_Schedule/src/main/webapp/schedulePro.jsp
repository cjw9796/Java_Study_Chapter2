<%@page import="com.calendar.MyCalendarTen"%>
...
<head>
<meta charset="UTF-8">
<title>파일명: schedulePro.jsp</title>
<style>
table {
		/* border: 선두께 선종류 선색상 */
	...
	%>
		...
		<tr>
			<th>
				...
				href 속성에 "#" 뒤에 id(해시)를 지정하면 현재 문서에서 id가 지정된 요소로 이동하고, 
				url(주소)이 지정되면 url 페이지로 이동한다.
				
				"?" 뒤에 이동하는 페이지로 전달할 데이터를 넘겨주는데 넘겨줄 데이터 2건 이상
				데이터와 데이터 구분할 때 "&"를 넣어서 구별한다.
		...
		// 달력을 출력할 달 1일의 요일을 계산해둔다.
				...
				if (month == 1) {
					...
					start = MyCalendarTen.lastDay(year, month - 1) - week; //2월~12월
				}

				// 1일이 출력될 위치(요일) 맞추기 위해 달력을 출력 할 달 1일의 요일만큼 반복하여 전달
				// 날짜를 출력한다. (빈칸에 데이터 출력하는 부분(5/28)(7/1)등)
				...
				for(int i = 1; i<= MyCalendarTen.lastDay(year, month); i++){
					...
					//  이부분에 scheduleMain에서 값 가져와서 출력 
						else if (month == 6 && i == 14) {
						out.println("<td class='schedule'>" + i + "<br><span>오늘</span></td>");
					}	else if (month == 6 && i == 15) {
						out.println("<td class='schedule'>" + i + "<br><span>공강</span></td>");
					}	else if (month == 6 && i == 16) {
						out.println("<td class='schedule'>" + i + "<br><span>공강</span></td>");
					}   
				...
				if(month == 12){
					...
					week = MyCalendarTen.weekDay(year,month +1, 1);  // 1월~11월 
				}
