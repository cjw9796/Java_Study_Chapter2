...
public class BoardDAO {
	...
	public void boardUpdate(BoardDTO board) {
		...
		boardlist.get(index).setEmail(board.getEmail()); // 이메일
		boardlist.get(index).setSubject(board.getSubject()); // 제목
		boardlist.get(index).setContent(board.getContent()); // 내용
	...
	public void boardReplyWrite(int num, BoardDTO board) {
		...
		//아무리 답글이더라도 새로운 내용이 추가됐기 때문에 idNum 글번호는 증가한다. 
