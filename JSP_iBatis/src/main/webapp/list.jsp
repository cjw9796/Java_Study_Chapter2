...
<body>
	<!--  
	 ...
	 <%
	 	...
	 	// 검색어가 넘어온 경우(내용,이름,내용+이름)와 넘어오지 않은 경우 메서드 실행을 
	 	// 각각 해야된다. 
	 	...
	 	// 검색어는 입력이 되었는데 카테고리 내용만 인경우
	 	//else if(category.equals("내용"))
	 	
	 	// 검색어는 입력이 되었는데 이름인 경우 
	 	//else if(category.equals("이름"))
	 	
	 	// 검색어는 입력이 됭었는데 이름+ 내용 인 경우
	 	//else if(category.equals("내용+이름"))
	 	
	 	// 검색어가 입력된 경우 검색어가 입력되지 않은 경우 구별해서 메서드를 호출
	 	if(item == null || item.trim().length() == 0){
	 		// 검색어X
	 		guestbookList = service.selectList(currentPage);
	 	}else{
	 		// 검색어 O
	 		guestbookList = service.selectListMulti(currentPage,
	 													category,item);
	 	}
