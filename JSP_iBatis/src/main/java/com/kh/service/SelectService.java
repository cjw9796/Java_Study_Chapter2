...
//		GuestbookDAO 클래스에 2번 접근 접근해서 sql 명령을 실행해야 하므로 
	...
	// selectByIdx.jsp 호출되는 수정 또는 삭제 글번호를 넘겨 받고 
	// mapper를 얻어온 후 DAO 클래스로 가서 글 1건을 얻어오는 select sql명령을 
	// 실행하는 메서드를 호출 한다.	
	...
	//list.jsp에서 호출되는 화면에 표시할 페이지 번호와 검색어 (내용+이름)를넘겨받아
	// mapper를 얻어온 후  DAO 클래스 1페이지 분량의 검색어를 포함하는 글 목록을
	// 얻어오는 select sql명령을 실행하는 메서드를 호출하는 메서드
	
	public GuestbookList selectListMulti(int currentPage, String category
										,String item) {
		
		System.out.println("SelectService의 selectListMulti() 메서드 실행");
		SqlMapClient mapper = MyAppSqlConfig.getSqlMapInstance();
		...
		try {
			...
			// 카테고리에 따른 검색어가 포함되었는지 조건을 세워야 하기 떄문에 
			// Param 클래스 객체를 이용한다. 
			...
			// 실제 북리스트에 저장하는 내용 검색된 개수를 가지고 
			// 실제 내용들을 (글 목록들을 가져오는 메서드를 만들어서 리턴 받는다.)
