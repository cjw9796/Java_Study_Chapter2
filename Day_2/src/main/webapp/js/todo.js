 ...
 */
 ...
 //이벤트 처리 
 //  DOMContentLoaded 브라우저 html 을 전부 읽고 DOM객체를 완성하는데 이미지 파일이나
// 스타일이 적용 태그들을 기다려야되는데 기다리지 않고 그냥 구조만 실행~!
 ...
 function addTodo(e){
	...
	newDiv.appendCild(completeButton);
	
	//삭제!
	const deleteButton = document.createElement('button');
	deleteButton.innerText="삭제";
	deleteButton.classList.add('deleteButton');
	newDiv.appendCild(deleteButton);
 ...
 function saveToLocal(todo){
	...
	//위에 데이터를 하나의 배열에 저장. 그래서 값을 가져와서 추가하는 항목은 실제 변경된 배열에 추가한다. 
...
function removeLocal(todo){
	...
	todo.splice(index,1); //index 번째 요소 삭제 
